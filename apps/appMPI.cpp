...
int main(int argc, char **argv)
{
    ...
    if (!myRank)
    {
        ...
        desc.add_options()("help", "produce help")("file", value<std::string>(), "image file")("kmean-value", value<int>()->default_value(0), "KMean k value")("rgb", value<int>()->default_value(1), "RGB or GrayScale");
        variables_map vm;
        ...
        if (vm["rgb"].as<int>() == 1)
        {
            image = imread(img_file.c_str(), IMREAD_COLOR); // Read the file in RGB mode
        }
        else
        {
            image = imread(img_file.c_str(), IMREAD_GRAYSCALE); // Read the file in GS mode
        }
        ...
        std::ofstream myFile;
        std::string logFile = "logKMeansMPI.csv";
        ifstream iFile;
        iFile.open(logFile);
