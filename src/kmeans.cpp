...
namespace KM
{
	...
	//Constructor
	KMeans::KMeans(uint16_t nb_channels, uint16_t nb_centroids)
	...
	void KMeans::setCentroids(std::vector<uchar> centr)
	{
		m_centroids = centr;
	...
	std::vector<uchar> KMeans::getCentroids() const
	{
		return (m_centroids);
	}
	...
	//pixels of the image
	int KMeans::compute_centroids(cv::Mat const &image,
								  int max_iterations,
								  double epsilon,
								  std::string mode)
	{
	...
	void KMeans::init_centroids(cv::Mat const &image)
	{
		using namespace cv;
	...
	std::vector<double> KMeans::update_centroid(cv::Mat const &image,
												std::string mode)
	{
		using namespace cv;
		using namespace tbb;
...
#pragma omp parallel for schedule(static, 8) reduction(+                                                \
	...
	//Closest centroid
	uint32_t KMeans::argClosest(double &distance,
								int row,
								int col,
								cv::Mat const &image)
	{
	...
	//Segmentation
	void KMeans::compute_segmentation(cv::Mat &image,
									  std::string mode)
	{
		using namespace cv;
		using namespace tbb;
...
#pragma omp parallel for firstprivate(k) schedule(static, 8) if (omp)
	...
	int KMeans::process(cv::Mat &image,
						int max_iterations,
						double epsilon,
						std::string mode)
	{
